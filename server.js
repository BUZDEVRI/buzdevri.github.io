...
// Boşta bekleyen oyuncuları tutacağımız sıra (SUNUCU GENELİNDE TEK OLMALI)
let beklemeSirasi = [];
...
io.on('connection', (socket) => {
    ...
    // Oyuncu bağlantısı koptuğunda hem oyunculardan hem kuyruktan silelim
    socket.on('disconnect', () => {
        console.log('Oyuncu ayrıldı:', socket.id);
        
        // Oyuncular dizisinden çıkar
        const index = oyuncular.findIndex(p => p.id === socket.id);
        if (index !== -1) {
            oyuncular.splice(index, 1);
        }

        // Bekleme sırasından çıkar
